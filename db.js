class MenapDB {
  ...
  getProfile() {
    ...
    if (!res.length || !res[0].values.length) return { user_id:'', first_name:'', last_name:'', email:'', password:'', photo:'' };
  ...
  saveProfile(data) {
    ...
    if (existing.first_name !== undefined && existing.first_name !== '') {
  ...
  updateProfileFull({ first_name, last_name, email, old_password, new_password, photo }) {
    ...
    if (email && email !== profile.email) {
      const existing = this._row(`SELECT id FROM profile WHERE email=?`, [email]);
      if (existing) throw new Error('email_exists');
    }
    if (new_password) {
      if (!old_password) throw new Error('old_password_required');
      const pwCheck = this._row(`SELECT id FROM profile WHERE id=? AND password=?`, [profile.id, old_password]);
    ...
    this.db.run(`UPDATE profile SET first_name=?,last_name=?,email=?,photo=?,password=? WHERE id=?`,
      [fn, ln, em, ph, pw, profile.id]);
    this.db.run(`UPDATE members SET first_name=?,last_name=?,email=?,photo=? WHERE user_id=?`,
      [fn, ln, em, ph, profile.user_id]);
    this._save();
    this._syncToServer();
  }
